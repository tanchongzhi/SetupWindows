...
Write-Host 'Network Authentication : Restrct null session access : Enabled'
...
Write-Host 'Network Authentication : Restrict anonymous access to named pipes and shares : Enabled'
Set-ItemProperty -Path 'HKLM:\System\CurrentControlSet\Control\Lsa' -Name RestrictAnonymous -Value 1 -Type DWord

Write-Host 'Network Authentication : Restrict anonymous sam access to named pipes and shares : Enabled'
Set-ItemProperty -Path 'HKLM:\System\CurrentControlSet\Control\Lsa' -Name RestrictAnonymousSam -Value 1 -Type DWord

Write-Host 'Network Authentication : Limit local account use of blank passwords to console logon only : Enabled'
