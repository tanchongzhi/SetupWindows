...
if (-not ('WindowsNativeMethods.NotifyMessage' -as [type])) {
    Add-Type -Namespace WindowsNativeMethods -Name NotifyMessage -MemberDefinition @'
...
function Send-EnvironmentChangedMessage {
    ...
    $null = [WindowsNativeMethods.NotifyMessage]::SendNotifyMessageW($HWND_BROADCAST, $WM_SETTINGCHANGE, [IntPtr]::Zero, 'Environment')
}
