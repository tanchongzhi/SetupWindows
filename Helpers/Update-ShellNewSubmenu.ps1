...
function Get-ShellNewSubmenuItem {
    foreach ($fileType in (Get-ShellNewSubmenuFileTypes)) {
        ...
        if ($ShellNewSubmenuPreservedFileTypes -inotcontains $fileType) {
...
function Add-ShellNewSubmenuItem {
    ...
    if ($ShellNewSubmenuPreservedFileTypes -icontains $FileType) {
    ...
    if (Test-Path -Path $shellNewKeyPath) {
        $conflictingMethods = @('NullFile', 'FileName', 'Data', 'Command') | Where-Object { Get-ItemProperty -Path $shellNewKeyPath -Name $_ -ErrorAction SilentlyContinue }
